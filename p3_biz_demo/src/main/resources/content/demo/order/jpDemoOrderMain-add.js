...
$(function(){
	...
	$("#jpDemoOrderCustom_add").click(function(){
		var tr = $("#add_jpDemoOrderCustom_template tr").clone();
		$("#jpDemoOrderCustom_table tbody").append(tr);
	...
	$("#jpDemoOrderCustom_del").click(function(){
		$("#jpDemoOrderCustom_table").find("input:checked").parent().parent().remove();   
        resetTrNum('jpDemoOrderCustom_table'); 
        return false;
	});
		...
		$(':input, select,button,a', this).each(function(){
			...
			if(id!=null){
				if (id.indexOf("#index#") >= 0){
					...
					if(nextLabel != null){
						nextLabel.attr("for",id.replace('#index#',i));
					}
				}else{
					var s = id.indexOf("[");
					var end = id.indexOf("]");
					var new_id = id.substring(s+1,end);
